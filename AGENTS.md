...
`SimPod\ClickHouseClient\Client\ClickHouseClient` from the container (the app
provides it via the toolkit's `ClickHouseClientFactory`) and building a
`ClickHouseBatchWriter` per table. The core binds neither tracker key; one source
owns each (compose several sinks with the core `Composite*Tracker`). `config/di.php`
is covered by `ConfigWiringTest`, not by cs/psalm/phpunit.
...
- Trackers depend on the toolkit's `ClickHouseWriterInterface` (injected), so unit
  tests use an in-memory `RecordingWriter` — no server needed for `composer build`.
- `flush()` writes the buffer then clears it; an empty buffer writes nothing; a
  failed write keeps the buffer (caller may retry).
...
- Integration test (`tests/Integration/ClickHouseIntegrationTest`) is skipped
  ...
  `ClickHouseMigrationRunner`. The app must register a `ClickHouseClient` (via the
  toolkit's `ClickHouseClientFactory`) in DI.
