...
/**
 * Buffers conversions in memory and writes them to ClickHouse on {@see flush()}.
 *
 ...
 * the conversions table with exactly {@see COLUMNS} in this order; the `ts`
 * column is filled server-side by a `DEFAULT now()`.
 *
 * @api
 */
final class ClickHouseConversionTracker implements ConversionTracker
{
    /**
     * Insert columns, in order. The conversions table also has a server-defaulted
     * `ts` column that is intentionally not written here.
     *
     * @var list<string>
     */
    public const array COLUMNS = ['experiment', 'variant', 'subject_id', 'goal', 'is_forced', 'is_fallback', 'environment'];
